...
class MarioEnvironment(Process):
    def __init__(
        ...
        self.daemon = True
        self.env = JoypadSpace(gym_super_mario_bros.make(env_id), SIMPLE_MOVEMENT)
    ...
    def run(self):
        ...
        while True:
            ...
            if done:
                self.lst.append([self.episode, self.rall, self.steps])
                if (self.dataframe == True):
                    df = pd.DataFrame(self.lst, columns=['Epoch', 'Total result', 'steps'])
                    df.to_csv(self.title+'{}.csv'.format(self.env_idx))
