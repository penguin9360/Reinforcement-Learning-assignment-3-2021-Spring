...
def run(training_mode, pretrained, number):
    title = "dqn_leaky"
    dataframe = True
    lst = []
    ...
    for ep_num in tqdm(range(num_episodes)):
        ...
        lst.append([ep_num+1, total_rewards[-1]])
        if (dataframe == True):
            df = pd.DataFrame(lst, columns=['Epoch', 'Total result'])
            df.to_csv(title+'{}.csv'.format(number))
...
for i in range(3):
    run(training_mode=False, pretrained=False, number=i)
